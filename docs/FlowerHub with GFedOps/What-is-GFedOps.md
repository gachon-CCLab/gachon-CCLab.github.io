---
layout: default
title: What is GFedOps?
parent: FlowerHub with GFedOps
nav_order: 2
---
# 🎯 What is GFedOps?
<br />

![image.png](../../../img/FedOps-Launcher/gfedops.png)

Link: <a href="https://flower.ai/profile/gfedops/apps/" target="_blank">FlowerHub</a> 

GFedOps is a collection of FedOps Apps developed by Gachon CCL and published on FlowerHub. If FlowerHub is like an app store for federated learning applications, GFedOps is the place where FedOps-related apps are shared.

Users can bring GFedOps Apps from FlowerHub, create a FedOps client project, and run it with FedOps Launcher. This makes it easier to join federated learning without building the client code from scratch.

Key points:

- GFedOps provides FedOps-ready federated learning examples and client apps.
- Users can import a GFedOps App from FlowerHub and prepare it as a local client project.
- The client trains on local data and exchanges model updates with the federated learning server.
- GFedOps Apps can be executed more easily through FedOps Launcher.